...
const TextToThumbnailCard = () => {
  const [prompt, setPrompt] = useState(
    "Cinematic portrait of a cyberpunk samurai, neon rain, detailed armor, 8k resolution."
  )
  ...
  return (
    <div ref={containerRef} className="relative flex items-center justify-center w-full h-14 sm:h-16">
      {cards.map((card, index) => (
        ...
        >
          <div className="relative w-full h-full overflow-hidden rounded-lg bg-neutral-900/90 backdrop-blur-sm border border-white/20 ring-1 ring-white/5 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.1)] hover:border-white/40 hover:shadow-[0_0_20px_rgba(255,141,0,0.4)]">
            <img src={card.src} alt="element" className="object-cover w-full h-full opacity-90 hover:opacity-100" />
          </div>
        </div>
      ))}
    </div>
...
const ImageToThumbnailCard = () => {
  ...
  return (
    <div 
      ...
      className="relative bg-neutral-900/80 shadow-2xl outline outline-[6px] outline-white/5 h-[365px] w-[340px] sm:h-[440px] sm:w-[410px] rounded-3xl overflow-hidden mx-2.5 xl:mx-0 backdrop-blur-md flex flex-col"
    >
      {/* RESTORED: Standard padding (p-5 sm:p-6) to ensure exact vertical alignment with Cards 1 & 3 */}
      <div className="relative p-5 sm:p-6 z-10 flex flex-col h-full w-full gap-2 justify-between">
        {/* Input: Image Stack */}
        <div className="relative z-20 flex flex-col gap-2 shrink-0">
          <div className="flex items-center justify-between text-xs font-medium uppercase tracking-wider text-neutral-500">
